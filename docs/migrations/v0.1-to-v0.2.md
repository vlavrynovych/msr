...
This guide helps you migrate from `migration-script-runner` v0.1.x to `@msr/core` v0.2.0.
...
| What | Before | After |
|------|--------|-------|
| **Package Name** | `migration-script-runner` | `@msr/core` |
...
// After
import { MigrationScriptExecutor, Config } from '@msr/core';
```
...
// After
const { MigrationScriptExecutor } = require('@msr/core');
```
...
// After
import { IMigrationScript, IMigrationInfo } from '@msr/core';
```
...
✅ **All APIs unchanged** - No breaking changes to functionality
✅ **Configuration works the same** - Same `Config` and `BackupConfig`
✅ **Migration scripts unchanged** - Existing migrations work as-is
✅ **Database handlers unchanged** - `IDatabaseMigrationHandler` is the same
✅ **Behavior unchanged** - Same backup/restore, same execution flow
...
**Replace:** `from '@msr/core'`

**Find:** `require('migration-script-runner')`
**Replace:** `require('@msr/core')`
...
find . -type f \( -name "*.ts" -o -name "*.js" \) -not -path "*/node_modules/*" \
  -exec sed -i '' 's/migration-script-runner/@msr\/core/g' {} +
```
...
Get-ChildItem -Recurse -Include *.ts,*.js -Exclude node_modules | ForEach-Object {
  (Get-Content $_) -replace 'migration-script-runner', '@msr/core' | Set-Content $_
}
```
...
⚠️ **Deprecated** - Marked as deprecated with migration message
🔒 **No longer maintained** - All development happens in `@msr/core`
📦 **Still available** - Won't be unpublished for stability
❌ **No updates** - No bug fixes or new features
...
- **npm:** [https://www.npmjs.com/package/@msr/core](https://www.npmjs.com/package/@msr/core)
