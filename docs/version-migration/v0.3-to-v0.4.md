...
- [ ] Rename `migrations` → `migrationRecords` in ISchemaVersion implementations
...
Here's a script to help migrate your codebase:

```bash
#!/bin/bash
# migrate-to-v0.4.sh

echo "Migrating codebase to MSR v0.4.0..."

# 1. Update filePattern to filePatterns
find . -name "*.ts" -type f -exec sed -i '' \
  's/config\.filePattern = /config.filePatterns = [/g' {} \;
find . -name "*.ts" -type f -exec sed -i '' \
  's/config\.filePattern=/config.filePatterns=[/g' {} \;

echo "✓ Updated filePattern → filePatterns"

# 2. Update migrateTo → up
find . -name "*.ts" -type f -exec sed -i '' \
  's/executor\.migrateTo(/executor.up(/g' {} \;

echo "✓ Updated migrateTo() → up()"

# 3. Update downTo → down
find . -name "*.ts" -type f -exec sed -i '' \
  's/executor\.downTo(/executor.down(/g' {} \;

echo "✓ Updated downTo() → down()"

# 4. Update ISchemaVersion property
find . -name "*.ts" -type f -exec sed -i '' \
  's/\.migrations\.getAll/\.migrationRecords.getAllExecuted/g' {} \;

echo "✓ Updated ISchemaVersion.migrations → migrationRecords"

echo ""
echo "⚠️  Manual steps still required:"
echo "  1. Add checkConnection() to your IDB implementation"
echo "  2. Close array brackets for filePatterns"
echo "  3. Review and test all changes"
echo "  4. Update any custom interface implementations"
```

**Usage:**
```bash
chmod +x migrate-to-v0.4.sh
./migrate-to-v0.4.sh
```

{: .warning }
Always review automated changes and test thoroughly before deploying.
...
- [SQL Migrations Guide](../user-guides/sql-migrations) - Complete SQL migration documentation
- [API Reference - Interfaces](../api/interfaces) - Updated interface documentation
- [API Reference - Core Classes](../api/core-classes) - Updated MigrationScriptExecutor API
- [Configuration](../configuration/) - All configuration options including new features
- [Previous Migration: v0.2 to v0.3](v0.2-to-v0.3) - If upgrading from earlier versions
